...
var ActivityService = /** @class */ (function () {
    ...
    ;
    ActivityService.prototype.GetActivityTrackerDocument = function (baseurl, contact) {
        return this.http.get(baseurl + "GetActivityTrackerDocument/" + contact);
    };
    ;
...
var WoAssignmentComponent = /** @class */ (function () {
    function WoAssignmentComponent(mapsAPILoader, ngZone, http, router, activatedRoute, commonService, workOrderService, activityService, maintenanceService, checklistformService, userService, _sanitizer, formBuilder, dateAdapter, datepipe, lightbox, lightboxEvent, lighboxConfig) {
            ...
            this.activatedRoute.queryParams.subscribe(function (params) {
                if (params['workID'] == undefined) {
                    ...
                    _this.uploader.onSuccessItem = function (item, response, status, headers) {
                        //this.showSwal("File has been uploaded successfully!");
    ...
    WoAssignmentComponent.prototype.activityDocShow = function () {
        ...
        this.getActivityTrackerDocument(localStorage.getItem("WorkOrderNo"));
    };
    ...
    WoAssignmentComponent.prototype.getWorkOrderAssignment = function (workOrdId) {
        ...
        this.workOrderService.GetWorkOrderAssignment(this.commonService.baseApiUrl, workOrdId).subscribe(function (data) {
            ...
            if (wAsgList != null && wAsgList.length > 0) {
                ...
                $('.loader').hide();
                for (var _i = 0, wAsgList_1 = wAsgList; _i < wAsgList_1.length; _i++) {
                    var woa = wAsgList_1[_i];
                    if (woa.IsDeleted == false || woa.IsDeleted == null) {
                        _this.isSend = false;
                    }
                    else {
                        _this.isSend = true;
    ...
    WoAssignmentComponent.prototype.getActivityTrackerDocument = function (workOrdNo) {
        var _this = this;
        $('.loader').show();
        this.activityService.GetActivityTrackerDocument(this.commonService.baseApiUrl, workOrdNo).subscribe(function (data) {
            var datawAct = JSON.stringify(data);
            var actParse = JSON.parse(datawAct);
            var actList = JSON.parse(actParse["_body"]);
            _this.workOrdActTrackerDocsList = null;
            if (actList != null) {
                _this.workOrdActTrackerDocsList = actList;
                console.log(_this.workOrdActTrackerDocsList);
                $('.loader').hide();
            }
        });
    };
    WoAssignmentComponent.prototype.createWorkOrderAssignment = function () {
        ...
        if (this.workOrdAssPage.valid) {
            ...
            this.workOrderService.CreateWorkOrderAssignment(this.commonService.baseApiUrl, contact).subscribe(function (data) {
                ...
                if (userInfo == "Ok") {
                    _this.getWorkOrderAssignment(localStorage.getItem("WorkOrderId"));
                    //this.showSwal("Work order has been assigned.");
                }
    ...
    WoAssignmentComponent.prototype.getChecklistFormWOAssignmentTaskAnswer = function (chkFWoID, userID) {
        var _this = this;
        $('.loader').show();
        this.checklistformService.GetChecklistFormWOAssignmentTaskAnswer(this.commonService.baseApiUrl, chkFWoID, localStorage.getItem("WorkOrderId"), userID).subscribe(function (data) {
            var datawAsg = JSON.stringify(data);
            var wAsgParse = JSON.parse(datawAsg);
            var wAsgList = JSON.parse(wAsgParse["_body"]);
            ...
            if (wAsgList != null) {
                ...
                _this.getChecklistDocument(chkFWoID, userID);
                $('.loader').hide();
            }
        });
    };
    WoAssignmentComponent.prototype.getChecklistDocument = function (chkFWoID, userID) {
        var _this = this;
        $('.loader').show();
        this.checklistformService.GetChecklistDocument(this.commonService.baseApiUrl, chkFWoID, userID, localStorage.getItem("WorkOrderId")).subscribe(function (data) {
            var datawAsg = JSON.stringify(data);
            var wAsgParse = JSON.parse(datawAsg);
            var wAsgList = JSON.parse(wAsgParse["_body"]);
            _this.chkFormDocumentList = null;
            if (wAsgList != null) {
                var imglen = wAsgList.length - 1;
                for (var i = 0; i <= imglen; i++) {
                    if (wAsgList[i].FilePath != null) {
                        var src = wAsgList[i].FilePath;
                        var caption = 'Image ' + i + ' caption here';
                        var thumb = wAsgList[i].FilePath;
                        var album = {
                            src: src,
                            caption: caption,
                            thumb: thumb
    ...
    WoAssignmentComponent.prototype.open = function (index) {
        var _this = this;
    ...
    WoAssignmentComponent.prototype._onReceivedEvent = function (event) {
        if (event.id === __WEBPACK_IMPORTED_MODULE_17_angular2_lightbox__["LIGHTBOX_EVENT"].CLOSE) {
            this.subscription.unsubscribe();
        }
    };
    WoAssignmentComponent.prototype.closeJobModal = function () {
        __WEBPACK_IMPORTED_MODULE_13_sweetalert2___default()({
            title: 'Are you sure?',
            //text: 'You will not be able to revert this!',
            type: 'warning',
    ...
    WoAssignmentComponent = __decorate([
        ...
        __metadata("design:paramtypes", [__WEBPACK_IMPORTED_MODULE_11__agm_core__["c" /* MapsAPILoader */],
            ...
            __WEBPACK_IMPORTED_MODULE_17_angular2_lightbox__["Lightbox"],
            __WEBPACK_IMPORTED_MODULE_17_angular2_lightbox__["LightboxEvent"],
            __WEBPACK_IMPORTED_MODULE_17_angular2_lightbox__["LightboxConfig"]])
